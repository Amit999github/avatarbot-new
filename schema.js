...
module.exports.deviceJoiSchema = myCustomJoi.object({
  deviceType: myCustomJoi
    .string()
    .valid('on_off_switch', 'fan_switch', 'others')
    .required()
    .messages({
      'any.only': 'Invalid device type selected',
      'string.empty': 'Device type is required',
    }),
  deviceName: myCustomJoi.string().min(3).max(50).required().messages({
  ...
  deviceId: myCustomJoi.string().min(2).max(50).messages({
...
module.exports.roomJoiSchema = myCustomJoi.object({
  room_name: myCustomJoi.string().trim().min(2).max(50).required().messages({
  ...
  userId: myCustomJoi.string().allow('', null),
  _csrf: myCustomJoi.string().optional(),
});
